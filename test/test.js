...
class TestRunner {
  ...
  async run() {
    ...
    console.log("\n" + "=".repeat(50));
    console.log(
      `Tests: ${colors.green}${this.passed} passed${colors.reset}, ${colors.red}${this.failed} failed${colors.reset}, ${this.passed + this.failed} total`,
    );
...
runner.test("Server should initialize and list tools", async function () {
  ...
  this.assertContains(
    toolNames,
    "search_memory",
    "Should have search_memory tool",
  );
  this.assertContains(
    toolNames,
    "delete_memory",
    "Should have delete_memory tool",
  );
...
runner.test("Should return cache statistics", async function () {
  ...
  if (this.redisClient) {
    this.assertContains(
      text,
      "cached_memories",
      "Should show cached memories count",
    );
    this.assertContains(
      text,
      "redis_memory_usage",
      "Should show Redis memory usage",
    );
    this.assertContains(
      text,
      "top_accessed",
      "Should show top accessed memories",
    );
  } else {
    this.assertContains(
      text,
...
runner.test("Should return sync status", async function () {
  ...
  this.assertContains(
    text,
    "pending_memories",
    "Should show pending memories count",
  );
});
...
runner.test("Should optimize cache", async function () {
  ...
  if (this.redisClient) {
    this.assertContains(
      result.content[0].text,
      "Cache optimized",
      "Should indicate optimization complete",
    );
    this.assertContains(
      result.content[0].text,
      "L1",
      "Should show L1 cache info",
    );
    this.assertContains(
      result.content[0].text,
      "L2",
      "Should show L2 cache info",
    );
  } else {
    this.assertContains(
      result.content[0].text,
