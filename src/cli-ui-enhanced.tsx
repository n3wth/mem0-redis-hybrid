...
const ASCIILogo: React.FC = () => (
  <Box flexDirection="column" alignItems="center" marginBottom={1}>
    ...
    <Text color="cyan" dimColor>
      Intelligent Memory System v{process.env.npm_package_version || "1.2.7"}
    </Text>
  </Box>
);
    ...
    <Box flexDirection="column" minHeight={15}>
      <Box flexDirection="row" justifyContent="space-between" marginBottom={1}>
        ...
        <Box flexDirection="column" width="35%" marginLeft={2}>
          ...
          <Text>
            Engine: <Text color="green">● Local</Text>
          </Text>
          <Text>
            Redis: <Text color="green">● Connected</Text>
          </Text>
        </Box>
      </Box>
          ...
          {memories
            ...
            .map((memory, idx) => {
              ...
              return (
                <Text
                  key={memory.id}
                  color={actualIdx === selectedIndex ? "cyan" : "white"}
                >
                  {actualIdx === selectedIndex ? "▶ " : "  "}[{actualIdx + 1}]{" "}
                  {(memory.content || "").substring(0, 65)}...
                </Text>
              );
            })}
        </Box>
            ...
            {memories
              ...
              .map((memory, idx) => {
                ...
                return (
                  <Text
                    key={memory.id}
                    color={actualIdx === selectedIndex ? "red" : "white"}
                  >
                    {actualIdx === selectedIndex ? "▶ " : "  "}[{actualIdx + 1}
                    ] {(memory.content || "").substring(0, 65)}...
                  </Text>
