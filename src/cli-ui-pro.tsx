...
const ActivityIndicator: React.FC<{ active: boolean }> = ({ active }) => {
  ...
  return (
    <Text color={active ? (pulse ? "green" : "greenBright") : "gray"}>●</Text>
  );
};
...
const MemoryManagerPro: React.FC<AppProps> = ({ memoryEngine }) => {
  ...
  useEffect(() => {
    const loadStats = async () => {
      try {
        ...
        const cacheStats = (engine as any).getCacheStats?.() || {
          hits: 0,
          misses: 0,
        };
        setStats({
  ...
  const searchMemories = async () => {
    ...
    try {
      ...
      setMessage(
        `Found ${transformedResults.length} results in ${searchTime}ms`,
      );
    } catch (err: any) {
      setError(err.message);
    } finally {
          ...
          <Box flexDirection="column">
            {memories.slice(0, 8).map((memory, idx) => (
              <Box key={memory.id}>
                <Text
                  color={
                    memory.similarity && memory.similarity > 0.8
                      ? "green"
                      : memory.similarity && memory.similarity > 0.6
                        ? "yellow"
                        : "white"
                  }
                >
    ...
    <Box flexDirection="column" minHeight={14}>
      <Box flexDirection="row" justifyContent="space-between" marginBottom={1}>
        <Box>
          ...
          <Text dimColor>
            {" "}
            • {memories.length} of {stats.total} loaded
          </Text>
        </Box>
        <Text dimColor>
