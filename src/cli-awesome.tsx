...
const AwesomeMemoryManager: React.FC = () => {
  ...
  const navigateMemories = useCallback(
    (direction: "up" | "down" | "first" | "last") => {
      setState((prev) => {
        const currentList =
          prev.view === "search" ? prev.searchResults : prev.memories;
  ...
  if (!isRawModeSupported) {
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <Text dimColor>
          Use this tool in a proper terminal for the full interactive experience
        </Text>
        <Text dimColor>
          Try: Terminal.app, iTerm2, or any standard terminal
        </Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" minHeight={20}>
      ...
      >
        ...
        <Text dimColor> | </Text>
        <Text color="blue">
          {Math.round(state.stats.cache.hitRate)}% cache hit
        </Text>
        <Text dimColor> | </Text>
      ...
      <Box flexGrow={1} flexDirection="row">
        ...
        <Box
          ...
          borderStyle="single"
          borderColor={
            state.view === "list" || state.view === "search" ? "cyan" : "gray"
          }
          marginRight={1}
          paddingX={1}
        >
            ...
            {(state.view === "search" ? state.searchResults : state.memories)
              ...
              .map((memory, index) => {
                ...
                return (
                  <Text
                    key={memory.id}
                    color={isSelected ? "green" : isDangerous ? "red" : "white"}
                  >
              ...
              <Box marginTop={1} flexDirection="column">
                ...
                <Box>
                  <Text dimColor>
                    Created:{" "}
                    {new Date(
                      state.selectedMemory.created_at,
                    ).toLocaleDateString()}
                  </Text>
                </Box>
                ...
                {state.selectedMemory.metadata.requires_sudo && (
                  <Box>
                    <Text color="yellow">{figures.warning} Requires sudo</Text>
                  </Box>
                )}
              </Box>
          ...
          {state.view === "stats" && (
            <>
              ...
              <Box marginTop={1} flexDirection="column">
                ...
                <Box marginTop={1}>
                  <Text color="cyan">Operation Times (ms):</Text>
                </Box>
                {Object.entries(state.stats.performance).map(
                  ([op, stats]: [string, any]) => (
                    <Text key={op}>
      ...
      <Box borderStyle="single" borderColor="dim" paddingX={1}>
        <Text dimColor>
          ...
          {state.selectedMemory
            ? ` Selected: ${state.selectedMemory.content.substring(0, 20)}...`
            : ""}
        </Text>
      </Box>
    </Box>
