...
const demoMemories = [
  ...
  {
    content:
      'curl -X POST https://api.example.com/webhook -H "Content-Type: application/json" -d \'{"event": "deploy", "status": "success"}\'',
    user_id: "oliver",
  ...
  {
    content:
      'ssh -i ~/.ssh/production.pem ubuntu@prod-server "tail -f /var/log/app.log"',
    user_id: "oliver",
    project: "production",
...
async function populateDemoData() {
  ...
  try {
    ...
    for (const memory of demoMemories) {
      ...
      console.log(
        `✓ Added memory: ${memory.content.substring(0, 50)}... (${id})`,
      );
    }
