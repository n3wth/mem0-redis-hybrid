    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box borderStyle="round" borderColor="cyan" paddingX={1}>
          <Text bold color="cyan">Memory Details</Text>
        </Box>
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Box marginTop={2} flexDirection="column">
            ...
            <Text dimColor>Created: {new Date(memory.created_at).toLocaleString()}</Text>
            {memory.augmented && <Text color="green">✓ Augmented</Text>}
            {memory.tags && memory.tags.length > 0 && (
              <Text>Tags: {memory.tags.map(t => chalk.yellow(`#${t}`)).join(' ')}</Text>
            )}
          </Box>
        </Box>
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box borderStyle="round" borderColor="magenta" paddingX={1}>
          <Text bold color="magenta">Exa Search Results</Text>
        </Box>

        {exaResults.map((result, i) => (
          <Box key={i} marginTop={1} flexDirection="column">
            <Text color="cyan" bold>[{i + 1}] {result.title}</Text>
            <Text dimColor wrap="wrap">{result.snippet}</Text>
            <Text color="blue" underline>{result.url}</Text>
          </Box>
        ))}
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box borderStyle="round" borderColor="cyan" paddingX={1} marginBottom={1}>
        ...
        <Text> | </Text>
        <Text color="blue">{memories.filter(m => m.augmented).length} augmented</Text>
      </Box>
