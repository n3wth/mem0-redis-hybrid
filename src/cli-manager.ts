...
class MemoryGraphManager {
  ...
  constructor() {
    ...
    // Create grid layout
    this.grid = (new (contrib as any).grid({ rows: 12, cols: 12, screen: this.screen }));
  ...
  private setupKeyBindings() {
    ...
    this.screen.key(['enter'], () => {
      if (this.selectedNode) {
        ...
        this.log(chalk.yellow('No memory selected. Use arrow keys to navigate.'));
      }
    });
  ...
  private switchPanel() {
    const panels: Array<'graph' | 'details' | 'stats' | 'search'> = ['graph', 'details', 'stats', 'search'];
  ...
  private async enterExaSearchMode() {
    if (!EXA_API_KEY) {
      ...
      this.log(chalk.yellow('Set EXA_API_KEY environment variable to use Exa search'));
  ...
  private async searchWithExa(query: string) {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        this.log(chalk.green(`Found ${this.exaResults.length} results from Exa`));
      } else {
  ...
  private async augmentMemoryWithExa() {
    ...
    this.log(chalk.gray(`Current content: ${memory.memory.substring(0, 50)}...`));
    ...
    this.commandLine.clearValue();
    this.commandLine.hidden = false;
    this.commandLine.setLabel(' Augment with Exa (edit query or press Enter for default) ');
  ...
  private async saveAugmentedMemory() {
    if (!this.memoryBeingAugmented || !this.exaResults || this.exaResults.length === 0) {
...
Updated: ${new Date().toISOString()}`;
    ...
    this.log(chalk.green(`✓ Memory augmented with ${this.exaResults.length} Exa results`));
    this.memoryBeingAugmented = null;
    this.refreshDisplay();
  }

  private async includeSpecificResult(index: number) {
    if (!this.memoryBeingAugmented || !this.exaResults || index >= this.exaResults.length) {
...
Updated: ${new Date().toISOString()}`;
  ...
  private navigateGraph(direction: string) {
    ...
    if (!this.selectedNode) {
      ...
      const currentIndex = memoryNodes.findIndex(n => n.id === this.selectedNode);
  ...
  private async exportData() {
    const exportPath = path.join(process.cwd(), `r3call-export-${Date.now()}.json`);
  ...
  private updateGraphDisplay() {
    ...
    // Group nodes by type
    const memoryNodes = this.graphNodes.filter(n => n.type === 'memory').slice(0, 10);
    const entityNodes = this.graphNodes.filter(n => n.type !== 'memory').slice(0, 10);
  ...
  private async loadMemories() {
    ...
    try {
      ...
      if (this.redisClient) {
        try {
          ...
          const memoryKeys = allKeys.filter(key => {
            ...
            return parts.length === 2 && !['keywords', 'access', 'search'].includes(parts[1]);
          });
          ...
          if (skippedCount > 0) {
            this.log(chalk.gray(`Filtered out ${skippedCount} non-memory entries`));
          }
      ...
      if (memoryNodes.length > 0 && !this.selectedNode) {
        this.selectedNode = memoryNodes[0].id;
        this.log(chalk.cyan(`Auto-selected first memory: ${memoryNodes[0].label}`));
      }
  ...
  private async loadSampleData() {
    ...
    this.memories = [
      ...
      },
      {
        ...
        metadata: {
          entities: {
            technologies: [{ text: 'r3call' }, { text: 'Redis' }, { text: 'Mem0' }],
  ...
  private async buildGraph() {
    ...
    for (const memory of this.memories) {
      ...
      if (memory.metadata?.entities) {
        for (const [type, entities] of Object.entries(memory.metadata.entities)) {
  ...
  private async initializeConnections() {
    ...
    this.log(chalk.gray('Vector search and entity extraction disabled for stability'));
  }
